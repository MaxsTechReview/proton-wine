...
# 16 KB-page kernels. See build-step-arm64ec.sh for the rationale.
ENABLE_16KB_PAGES=0
for _arg in "$@"; do
  case "$_arg" in
...
if [ "${USE_CCACHE:-0}" = "1" ] && command -v ccache >/dev/null 2>&1; then
  ...
  CCWRAP="$HOME/.ccache-wrap-x86_64"
  mkdir -p "$CCWRAP"
...
do
  ...
  then
    ...
    if [ -d "$PROJECT_ROOT/android/android_sysvshm" ]; then
        ...
        # Propagate the parent build's NDK API level + page-size flags so
        # libandroid-sysvshm.so has the same alignment metadata as the rest
        # of the package (see build-step-arm64ec.sh's matching block).
