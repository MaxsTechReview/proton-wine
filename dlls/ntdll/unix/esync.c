...
/* WinNative: set to 1 by esync_init() on shm_open failure so the process
 * degrades to NTSync / server sync instead of exit(1)'ing. */
static int esync_runtime_disabled;
                             ...
                             BOOLEAN alertable, const LARGE_INTEGER *timeout )
{
    ...
    {
        TRACE("WINNATIVE_ESYNCWAIT MIXED esync+server count=%lu wait_any=%d alertable=%d msgwait=%d\n",
              (unsigned long)count, wait_any, alertable, msgwait);
        if (TRACE_ON(esync))
            for (i = 0; i < count; i++)
                TRACE("WINNATIVE_ESYNCWAIT MIXED[%d] handle=%p type=%d fd=%d\n", i,
                      handles[i], objs[i] ? objs[i]->type : -1, objs[i] ? objs[i]->fd : -1);
    }
    else if (has_server)
    {
        TRACE("WINNATIVE_ESYNCWAIT ALL-SERVER count=%lu (falling back to server wait path)\n",
              (unsigned long)count);
        if (TRACE_ON(esync))
            for (i = 0; i < count; i++)
                TRACE("WINNATIVE_ESYNCWAIT ALL-SERVER[%d] handle=%p\n", i, handles[i]);
        return STATUS_NOT_IMPLEMENTED;
    }
        ...
        while (1)
        {
            ...
            if (wn_sync_trace_enabled())
            {
                ...
                int n = snprintf( buf, sizeof(buf),
                                  "WN_SYNC_TRACE: esync_poll_ret tid=%04x ret=%d pollcount=%u count=%lu msgwait=%d",
                                  (unsigned)GetCurrentThreadId(), ret, pollcount,
                                  (unsigned long)count, msgwait );
                unsigned j;
                for (j = 0; j < pollcount && n < (int)sizeof(buf) - 32; ++j)
                    n += snprintf( buf + n, sizeof(buf) - n, " [%u]fd=%d rev=%#x",
...
void esync_init(void)
{
    ...
    if (!do_esync())
    {
        /* WinNative: upstream used to probe the server here and exit(1) if the
         * server had esync but the client didn't. That belt-and-suspenders
         * check turned into a hang when env var inheritance was imperfect
         * (any child process with WINEESYNC not set would kill itself even
         * though modern Wine handles mixed esync/non-esync cleanly). Keep the
         * probe as a debug warning only; don't exit. */
        HANDLE handle;
    ...
    {
        /* WinNative: degrade to NTSync / server sync instead of killing the
         * process. Upstream's exit(1) on first boot was the source of the
         * "stuck initializing, works after restart" symptom — the prefix
         * init's wineserver races with the client and one side wins the shm
         * race; losing meant exit(1) and a dead container. */
        if (errno == ENOENT)
            WARN("esync shm file not found (%s); disabling ESYNC for this process, falling back to NTSync/server sync.\n", shm_name);
        else
            WARN("Failed to map esync shared memory (%s); disabling ESYNC for this process: %s\n", shm_name, strerror( errno ));
        esync_runtime_disabled = 1;
        shm_fd = -1;
        return;
    }
